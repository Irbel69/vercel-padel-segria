  ...
  return (
    ...
    >
      <head>
        ...
        <meta
          name="description"
          content={metadata?.description ?? config.appDescription ?? ""}
        />
        <meta
          name="author"
          content={config?.resend?.fromAdmin ?? config.appName}
        />
        {/* Robots / indexing */}
        ...
        {(metadata as any)?.url || config.domainName ? (
          <link
            rel="canonical"
            href={(metadata as any)?.url ?? `https://${config.domainName}`}
          />
        ) : null}
        {/* Open Graph */}
        ...
        <meta
          property="og:title"
          content={(metadata as any)?.title ?? config.appName}
        />
        <meta
          property="og:description"
          content={(metadata as any)?.description ?? config.appDescription}
        />
        <meta
          property="og:type"
          content={(metadata as any)?.type ?? "website"}
        />
        <meta
          property="og:url"
          content={(metadata as any)?.url ?? `https://${config.domainName}`}
        />
        {(metadata as any)?.openGraph?.images ? (
          <meta
            property="og:image"
            content={(metadata as any).openGraph.images[0]}
          />
        ) : (
          <meta
            property="og:image"
            content={`https://${config.domainName}/opengraph-image.png`}
          />
        )}
        {/* Twitter Card */}
        <meta
          name="twitter:card"
          content={(metadata as any)?.twitter?.card ?? "summary_large_image"}
        />
        <meta
          name="twitter:title"
          content={(metadata as any)?.title ?? config.appName}
        ...
        <meta
          name="twitter:image"
          content={
            (metadata as any)?.openGraph?.images?.[0] ??
            `https://${config.domainName}/opengraph-image.png`
          }
        />
        {/* Microsoft Tiles */}
        <meta
          name="msapplication-TileColor"
          content={config.colors?.main ?? "#000000"}
        />
        <link
          rel="mask-icon"
          href="/icons/icon-192x192.png"
          color={config.colors?.main ?? "#000000"}
        />
      ...
      >
        ...
        >
          ...
          <div
            className="relative min-h-screen w-full bg-black overflow-x-hidden overflow-y-hidden"
            style={{
