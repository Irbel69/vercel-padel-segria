...
export default function TournamentsPage() {
	...
	// Registration choice UI state
	const [registerChoiceEventId, setRegisterChoiceEventId] = useState<
		number | null
	>(null);
	...
	const retryFocus = (attempts = 6, delay = 120) => {
		...
		const tick = () => {
			...
			if (
				tries < attempts &&
				(!joinCodeInputRef.current ||
					document.activeElement !== joinCodeInputRef.current)
			) {
				try {
	...
	return (
		<div className="space-y-3 md:space-y-6 px-2 md:px-0 max-w-full overflow-hidden">
											...
											<div className="md:hidden max-w-full">
												...
												<div className="pt-2 max-w-full">
													...
													{canRegister(event) && (
														<Button
															onClick={() => {
																setRegisterChoiceEventId(event.id);
															}}
			...
			<Dialog
				...
				<DialogContent
					...
					<DialogHeader>
						...
						<DialogDescription>
							Crea una invitació per inscriure't amb parella. Copia el codi o
							envia'l per correu.
						</DialogDescription>
					</DialogHeader>
						...
						<div className="border-t border-white/10 pt-4 space-y-3">
							<div className="text-sm">
								<label className="text-white/80 font-medium">
									Enviar per email (opcional)
								</label>
								<p className="text-xs text-white/60 mt-1">
									Envia directament la invitació al correu de la teva parella
								</p>
							</div>
			...
			{/* Register choice dialog */}
			<Dialog
				open={registerChoiceEventId !== null}
				onOpenChange={(open) => {
					if (!open) setRegisterChoiceEventId(null);
				}}>
				<DialogContent className="bg-zinc-900/90 backdrop-blur-md border-white/10 text-white max-w-sm">
					<DialogHeader>
						<DialogTitle>Com vols inscriure't?</DialogTitle>
						<DialogDescription>
							Pots inscriure't tu sol o bé iniciar una inscripció en parella.
						</DialogDescription>
					</DialogHeader>
					<div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
						<Button
							...
							onClick={async () => {
								const id = registerChoiceEventId;
								if (!id) return;
								const ok = await handleRegister(id);
								if (ok) {
									setRegisterChoiceEventId(null);
									toast({
										title: "Inscrit en solitari",
										description: "Has estat inscrit correctament",
									});
								}
							}}
							className="bg-padel-primary text-black hover:bg-padel-primary/90 font-semibold">
							{registerMutation.isPending ? (
								<span className="inline-flex items-center gap-2">
									<Loader2 className="h-4 w-4 animate-spin" /> Inscrivint...
								</span>
							) : (
								"En solitari"
							)}
						...
						<Button
							...
							onClick={() => {
								const id = registerChoiceEventId;
								if (!id) return;
								// Open pair invite dialog (existing flow)
					...
					<DialogFooter>
						<Button
							variant="outline"
							onClick={() => setRegisterChoiceEventId(null)}
							className="bg-white/10 border-white/20 text-white hover:bg-white/20">
							Cancel·lar
						</Button>
					</DialogFooter>
			...
			<Dialog
				...
				<DialogContent
					...
					<DialogHeader>
						...
						<DialogDescription>
							Introdueix el codi d'invitació que t'ha compartit la teva parella
							per unir-te al torneig.
						</DialogDescription>
					</DialogHeader>
					<div className="space-y-3">
						{(() => {
							...
							return (
								<div>
									<Input
										...
										aria-invalid={
											!!joinError || (!isValid && joinCode.length > 0)
										}
										...
										className={
											...
											`border ${
												joinError || (!isValid && joinCode.length > 0)
													...
													: "border-white/20 focus-visible:ring-padel-primary"
											}`
										}
										maxLength={6}
									/>
									<p
										...
										className={`mt-2 text-xs ${
											joinError ? "text-red-400" : "text-white/60"
										}`}>
										{joinError
											? joinError
											: "El codi té 6 caràcters (lletres i números). Exemple: ABC123"}
									</p>
								</div>
							);
						})()}
