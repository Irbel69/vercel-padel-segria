...
async function handler(req: NextRequest) {
	try {
		...
		const fetchRecentFormForUsers = async (
			userIds: string[],
			perUser: number = 5
		) => {
			...
			// For each user, sort by match_date desc and map to recent_form
			const result = new Map<string, RecentFormEntry[]>();
			rowsByUser.forEach((rows, uid) => {
				rows.sort((a, b) => {
					const da = Date.parse((a as any).matches?.match_date ?? 0);
					const db = Date.parse((b as any).matches?.match_date ?? 0);
					return db - da; // newest first
				});
				const entries: RecentFormEntry[] = [];
				for (const row of rows) {
					if (entries.length >= perUser) break;
					const pos = Number(row.position);
					const winner = (row as any).matches?.winner_pair as 1 | 2 | null;
					if (winner !== 1 && winner !== 2) {
						entries.push("N");
				...
				if (!userError && userData) {
					...
					if (!higherErr) {
						...
						if (superiorData && superiorData.length > 0)
							contextIds.push(superiorData[0].id);
						if (inferiorData && inferiorData.length > 0)
							contextIds.push(inferiorData[0].id);
