...
export async function POST(request: Request) {
	...
	try {
		...
		type BookingRow = {
			...
			status: string;
			created_at: string;
			lesson_slots: SlotRow;
			users: { id: string; name: string | null; email: string };
		};

		const filteredBookings = ((bookings || []) as BookingRow[]).filter(
			(booking) => {
				const slot = booking.lesson_slots as SlotRow;
				...
				if (body.date_range) {
					if (
						slotDate < body.date_range.from ||
						slotDate > body.date_range.to
					) {
						return false;
					}
				}
				...
				if (body.time_range) {
					...
					if (
						slotTime < body.time_range.start ||
						slotTime >= body.time_range.end
					) {
						return false;
					}
				}
		...
		const bookingsByDate = filteredBookings.reduce((acc, booking) => {
			const date = new Date(booking.lesson_slots.start_at)
				.toISOString()
				.slice(0, 10);
			if (!acc[date]) {
				acc[date] = [];
