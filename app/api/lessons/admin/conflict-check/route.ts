  ...
  try {
    ...
    for (const rule of rules || []) {
      ...
      conflictReasons.push(
        `Dies coincidents: ${overlappingDays
          .map((d) => getDayName(d))
          .join(", ")}`
      );

      conflictReasons.push(
        `Horari coincident: ${formatTimeOverlap(
          newRuleStart,
          newRuleEnd,
          existingStart,
          existingEnd
    ...
    const totalAffected =
      affectedBookings?.protection_summary?.total_affected_bookings ?? 0;
...
function dateRangesOverlap(
  ...
  if (overlaps) {
    ...
    if (from1 || from2)
      conflictDates.push(conflictStart.toISOString().slice(0, 10));
...
async function checkAffectedBookings(
  supabase: any,
  ruleData: ConflictCheckRequest
) {
  try {
    ...
    // With simplified booking statuses, only 'confirmed' and 'cancelled' exist.
    // Consider only 'confirmed' as protected; there are no modifiable ("pending") bookings anymore.
    const protectedBookings = filteredBookings.filter(
      (booking: any) => booking.status === "confirmed"
    );

    const modifiableBookings: any[] = [];
