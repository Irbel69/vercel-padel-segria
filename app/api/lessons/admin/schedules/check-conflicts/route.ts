...
import type { ScheduleTemplate, ScheduleBlock, ScheduleConflictResult, SlotConflict } from "@/types/lessons";
  ...
  for (let ts = from.getTime(); ts <= to.getTime(); ts += dayMs) {
    ...
    for (const block of body.template.blocks) {
      if (block.kind === 'lesson') {
        ...
        // break block
        const end = new Date(cursor.getTime() + block.duration_minutes * 60 * 1000);
        cursor = end;
      }
    }
  }
