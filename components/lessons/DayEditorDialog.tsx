...
export default function DayEditorDialog({
  ...
  const weekday = useMemo(() => {
    if (!day) return 0;
    // Use local day-of-week for the selected calendar day (0=Sunday..6=Saturday)
    // Avoid using toISOString() which can shift the date due to timezone.
  ...
  const buildPayload = () => {
    if (!day) return null;
    // Build YYYY-MM-DD from local calendar date to avoid TZ shifting
