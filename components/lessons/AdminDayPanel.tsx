    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      ...
      >
        <SheetHeader className="flex flex-row items-center justify-between">
          <SheetTitle className="capitalize">{title || "Selecciona un dia"}</SheetTitle>
        </SheetHeader>

        <div className="mt-4 space-y-2">
          {!day || items.length === 0 ? (
            <div className="text-white/70 text-sm">No hi ha classes per a aquest dia.</div>
          ) : (
            items.map((item, idx) => {
              if (item.kind === "break") {
